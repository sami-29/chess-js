...
function putpieces() {
    if(!ConfigObj.normal) {
        return ConfigObj.CUSTOM
    }
    return ConfigObj.BOARD
}
function drawpieces(CasesState) {
    for (let i = 0; i < 8; i++){
        for (let j = 0; j < 8; j++){
...
function rotateBoard() {
    if (!ConfigObj.whiteOnTop && player === 'white') {
...
socket.on('requeststart', (Data) => {
    CasesState = Data
    BlackandWhite()
    drawpieces(CasesState)
    socket.on('private', color => {
        player = color
        console.log(player);
        rotateBoard()
        rotateImgs()
...
sendHttpRequest('GET', 'json/config.json')
    .then((Data) => {
        ConfigObj = Data
        sendHttpRequest('GET', 'json/pieces.json')
            .then((Data) => {
                PiecesObj = Data
                CasesState = putpieces()
                socket.emit('start', CasesState)
